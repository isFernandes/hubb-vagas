...
export class PaymentsService {
  ...
  async createPreference(jobId: string, appId: string, price: number): Promise<string> {
    const preference = new Preference(this.client);
    const result = await preference.create({
      body: {
        items: [{ id: `${jobId}:${appId}`, title: 'Contratação de Candidato', quantity: 1, unit_price: price }],
  ...
  async verifyPayment(paymentId: string): Promise<{ approved: boolean; jobId: string; appId: string } | null> {
    try {
